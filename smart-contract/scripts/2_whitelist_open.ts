...
async function main() {
  ...
  if (
    CollectionConfig.whitelistSale.additionalSupply > 0 &&
  ...
  if (
    !(await contract.saleStatus()).eq(CollectionConfig.whitelistSale.saleStatus)
  ) {
    console.log(
      `Setting the sale status ${CollectionConfig.whitelistSale.saleStatus}...`
    );

    await await contract.setSaleStatus(
      CollectionConfig.whitelistSale.saleStatus
    );
  }
